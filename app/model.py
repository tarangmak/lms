...
def get_total_students():
    """
    Get total count of students (all users) from the database.
    Returns (success, message, count)
    """
    try:
        conn = get_conn()
        cur = conn.cursor(dictionary=True)
        sql_str = "SELECT COUNT(*) AS total FROM Users"
        cur.execute(sql_str)
        result = cur.fetchone()
        cur.close()
        conn.close()
        total = result["total"] if result else 0
        return (True, "Total students retrieved successfully", total)
    except Error as e:
...
def get_total_courses():
    """
    Get total count of courses from the database.
    Returns (success, message, count)
    """
    try:
        conn = get_conn()
        cur = conn.cursor(dictionary=True)
        sql_str = "SELECT COUNT(*) AS total FROM Courses"
        cur.execute(sql_str)
        result = cur.fetchone()
        cur.close()
        conn.close()
        total = result["total"] if result else 0
        return (True, "Total courses retrieved successfully", total)
    except Error as e:
        return (False, str(e), 0)
